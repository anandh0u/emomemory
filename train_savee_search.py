"""Search stronger SAVEE binary audio classifiers with cached features."""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Search SAVEE binary classifiers.")
    ...
    parser.add_argument("--feature-cache", type=Path, default=Path("artifacts/savee_binary_features.joblib"))
    parser.add_argument("--model-out", type=Path, default=Path("artifacts/savee_binary_best_search.joblib"))
...
def main() -> None:
    ...
    best: tuple[tuple[float, float, float], dict[str, Any], object] | None = None
    ...
    print(f"Saved SAVEE search model to {args.model_out}", flush=True)
...
def load_or_build_features(args: argparse.Namespace) -> dict[str, np.ndarray]:
    ...
    train_records, val_records, test_records = map(to_binary_records, [dataset.train, dataset.val, dataset.test])
    features = {
        "X_train": records_to_features_progress(train_records, "train", sample_rate=args.sample_rate),
        "y_train": records_to_labels(train_records),
        "X_val": records_to_features_progress(val_records, "validation", sample_rate=args.sample_rate),
        "y_val": records_to_labels(val_records),
        "X_test": records_to_features_progress(test_records, "test", sample_rate=args.sample_rate),
...
def to_binary_records(records: list[object]) -> list[object]:
    converted = []
...
def build_candidates(random_state: int) -> list[tuple[str, object]]:
    ...
    for c_value in [0.1, 0.3, 1.0, 3.0, 8.0, 15.0]:
        candidates.append(
            (
                f"logreg_C{c_value}",
                Pipeline(
